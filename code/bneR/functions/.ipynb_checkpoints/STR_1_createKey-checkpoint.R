...
createKey <- function(ref.df, refName, baseModel.df, baseModelName, poll, out_file) {
  ...
  baseModelLoc.df <- baseModel.df %>% 
    dplyr::mutate(baseModel_id = cell_id) %>% 
  ...
  refLoc.sf <- refLoc.df %>%
    ...
    sf::st_transform(crs = sf::st_crs("epsg:9311"))
  ...
  # 3d. save results
  key_ref_baseModel %>% 
    fst::write_fst(paste0(out_file, '.fst'))
}
