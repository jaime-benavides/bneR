function [space, time, preds, obs, num_points] = extract_components(dataset, inp_base_model_names, time_var)
% % 
% % === Inputs ===
% %  
...
if strcmp(time_var, 'percentOfYear')
    time = dataset.percent_of_year;
    
    if any(strcmp('julian_day', dataset.Properties.VariableNames))
        dataset.julian_day = [];
    end
    if any(strcmp('yyyy', dataset.Properties.VariableNames))
        dataset.yyyy = [];
    end
    
elseif strcmp(time_var, 'julianDay')
    time = dataset.julian_day;
    
    if any(strcmp('percent_of_year', dataset.Properties.VariableNames))
