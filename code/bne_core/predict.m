 function [mse_partial, r2_partial, cover_partial, me_partial, ...
    ...
    target, total_train_obs, predict_goal, inp_base_model_names, ...
...
%%%% ------------------------------ %%%%
 ...
 % This may need to be changed for daily 
 % window = 'daily'; num_models = 4; trainFold = 'all'; predFold = 'refGridConus'; 
...
[targetSpace, targetTime, targetPreds, targetObs, num_points] = extract_components(target, inp_base_model_names, time_var);
num_models = numel(inp_base_model_names);
%%%% -------------------------------- %%%%
...
for i = 1:num_points
    ...
    if strcmp(time_var, 'percentOfYear') % CARLOS-- Changed name to percentOfYear, consistent with config file % 
...
weights_mean = splitvars(table(softmax_mean));
weights_mean.Properties.VariableNames = append("w_mean_", inp_base_model_names);

weights_sd = splitvars(table(softmax_sd));
weights_sd.Properties.VariableNames = append("w_sd_", inp_base_model_names);

contribs = splitvars(table(contrib_sd));
contribs.Properties.VariableNames = append("contrib_sd_", inp_base_model_names);

preds = splitvars(table(targetPreds));
preds.Properties.VariableNames = append("pred_", inp_base_model_names);
...
results = [otherparam weights_mean weights_sd contribs preds];
